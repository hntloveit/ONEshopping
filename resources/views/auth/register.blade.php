...
<div class="page-ath-form">
    ...
    <form class="register-form" method="POST" action="{{ route('register') }}" id="register">
		...
		<div class="input-item">
            <input type="radio" class="input-" name="role" value="sale" id="sale"><label style="margin-left:10px" for="sale">Only Sale</label> &nbsp; &nbsp; &nbsp; &nbsp;
            <input type="radio" class="input-" name="role" value="user" checked id="ref"><label style="margin-left:10px" for="ref">Referral</label><br> 
        </div>
        @if( gws('referral_info_show')==1 && get_refer_id() )
        <div class="input-item">
            <input type="text" class="input-bordered" name="" value="{{ __('Your were invited by :userid', ['userid' => get_refer_id(true)]) }}" disabled readonly>
			<input name="referid" value="1" type="hidden" value="{{get_refer_id(true)}}">
		</div>
        @endif
