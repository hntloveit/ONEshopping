    ...
    @endif
	 ...
	 <script>
      jQuery( document ).ready(function() {
		...
		var checkonoff = jQuery('#checkonoff').val();
		var checknum = jQuery('#checknum').val();
		if(checkonoff == 0 || checknum == 6)
          jQuery('#spin').hide();
		...
		jQuery( ".showimg" ).click(function() {
		  ...
		  //setInterval(function(){
			...
			jQuery('.showgif').hide();
			jQuery.ajax({
			  url: '{!! action('User\OnepointController@onepoint') !!}',
			  type: 'GET',
			  dataType: "json",
			  data: '',
			  success: function(response){
				  ...
				  if(response.status == "success"){
					  jQuery('.point').html(response.value);
					  jQuery('.token-balance-text .lead b').html(0);
					  jQuery('#spin').hide();
					  
				  }else{
					  
					  console.log('success', 'Order failed added to cart');
