...
Route::prefix('user')->middleware(['auth', 'user', 'verify_user', 'g2fa'])->name('user.')->group(function () {
    ...
    Route::get('/referral/detail/{id}', 'User\UserController@referraldetail')->name('referraldetail');
    // My Token v1.1.2
    Route::get('/account/balance', 'User\UserController@mytoken_balance')->name('token.balance');
    Route::get('/onepoint', 'User\OnepointController@onepoint')->name('onepoint');
    Route::get('/cron_updatepoint_to_one', 'User\UserController@cron_updatepoint_to_one')->name('cron_updatepoint_to_one');
    Route::get('/sendonetouser', 'User\UserController@sendonetouser')->name('sendonetouser');
    
	Route::get('/sendone', 'User\UserController@sendone')->name('sendone');
	Route::post('/sendone', 'User\UserController@sendone')->name('sendone');
	Route::get('/ajax-tree', 'User\UserController@getAjaxTree');
	Route::get('/ajax-tree/{id}', 'User\UserController@getAjaxTreeID');
    ...
    Route::name('ajax.')->prefix('ajax')->group(function () {
		...
		Route::post('/sendone', 'User\UserController@sendone')->name('sendone');
		Route::get('/sendone', 'User\UserController@sendone')->name('sendone');
		Route::post('/confirmsendone', 'User\UserController@confirmsendone')->name('confirmsendone');
		Route::get('/confirmsendone', 'User\UserController@confirmsendone')->name('confirmsendone');
	});
});
...
Route::prefix('admin')->middleware(['auth', 'admin', 'g2fa', 'ico'])->name('admin.')->group(function () {
    ...
    Route::get('/settings/pointone', 'Admin\SettingController@pointone_setting')->middleware(['ico', 'super_admin'])->name('settings.pointone'); 
