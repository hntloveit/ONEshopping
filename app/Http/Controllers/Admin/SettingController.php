...
class SettingController extends Controller
{
    ...
    public function referral_setting() {
		...
		$ref_number = Setting::where('field','referral_number')->first();
		$referral_number = $ref_number->value;
        return view('admin.settings-referral', compact('general', 'advanced', 'referral_number'));
    }
	public function pointone_setting() {
        $onepoint = array();  
        return view('admin.settings-pointone', compact('onepoint'));
    }
    ...
    public function update(Request $request) {
		...
		if ($type == 'one') {
            $setting = Setting::updateValue('numpercent', $request->input('numpercent'));
			$on_offonetransfer = 0;
			if (isset($request['on_offonetransfer'])) {
                $on_offonetransfer = 1;
            }
            $setting = Setting::updateValue('on_offonetransfer', $on_offonetransfer);
            $ret['msg'] = 'success';
            $ret['message'] = __('messages.update.success', ['what' => 'Number']);
        }
		if ($type == 'refdetail') {
			$ref_number = Setting::where('field','referral_number')->first();
			$referral_number = $ref_number->value;
			$ret['msg'] = 'success';
            $ret['message'] = __('messages.update.success', ['what' => 'error']);
			$data = $request->all();
			for($i=1;$i<=$referral_number;$i++){
				$num = 'referral'.$i;
				$val = $data['referral'.$i];
				$setting = Setting::updateValue($num, $val);
			}
			
            $ret['msg'] = 'success';
            $ret['message'] = __('messages.update.success', ['what' => 'Number percent of bonus']);
        }
		if ($type == 'point') {
            
			$setting = Setting::updateValue('num1', $request->input('num1'));
            $setting = Setting::updateValue('num2', $request->input('num2'));
            $setting = Setting::updateValue('num3', $request->input('num3'));
            $setting = Setting::updateValue('num4', $request->input('num4'));
            $setting = Setting::updateValue('num5', $request->input('num5'));
            $setting = Setting::updateValue('num6', $request->input('num6'));
            $ret['msg'] = 'success';
            $ret['message'] = __('messages.update.success', ['what' => 'Number percent']);
        }
        ...
        if ($type == 'site_info') {
            ...
            if ($validator->fails()) {
                ...
                Setting::updateValue('referral_number', ($request->referral_number));
