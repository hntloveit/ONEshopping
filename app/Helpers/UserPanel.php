...
class UserPanel
{
    ...
    public static function user_balance_card($data = null, $atttr = '')
    {
		...
		$addxoay = '';
		if($class == 'card-full-height' && $user->tokenBalance > 0) 
			$addxoay = '<div id="spin">
			<div class="showimg"  ></div>
			<div class="showgif hide"  ></div>
			</div>'; 
        ...
        <span class="lead"><b>' . to_num_token($user->tokenBalance) . ' </b><span> ' . token('symbol') . ' <em class="fas fa-info-circle fs-11" data-toggle="tooltip" data-placement="right" title="'.__('Equivalent to'). ' ' .token_price($user->tokenBalance, base_currency()).' '.base_currency(true).'"></em></span></span>
        </div>
        </div>' . $contribute . '</div>'.$addxoay.'</div>';
